            ...
            <div className={`transition-all duration-500 ease-in-out overflow-hidden ${isOpen ? 'max-h-screen' : 'max-h-0'}`}>
                <div className="p-4 sm:p-6 border-t border-gray-700">
                    ...
                    <div className="text-xs text-gray-400 mt-4 pt-4 border-t border-gray-700 space-y-2">
                        ...
                        {order.receivedBy && order.receptionDate && (<p>Reçu par <span className="font-medium text-white">{getDisplayName(order.receivedBy?.email)}</span> le {new Date(order.receptionDate).toLocaleDateString('fr-FR')}</p>)}
                        {order.notifiedBy && order.notificationDate && (<p>Client prévenu par <span className="font-medium text-white">{getDisplayName(order.notifiedBy?.email)}</span> le {new Date(order.notificationDate).toLocaleDateString('fr-FR')}</p>)}
                        {order.pickedUpBy && order.pickedUpDate && (<p>Retiré par <span className="font-medium text-white">{getDisplayName(order.pickedUpBy?.email)}</span> le {new Date(order.pickedUpDate).toLocaleDateString('fr-FR')}</p>)}
                    </div>
