...
const OrderCard = ({ order, onRequestItemStatusUpdate, onCancelItem, onRequestOrderStatusUpdate, isAdmin, onShowHistory, onEdit, onRequestDelete, onInitiateRollback, onNotifyClient, isOpen, onToggleOpen, isNew }) => {
    ...
    const isReNotifyLocked = isNotified && hoursSinceNotification < 24;
    const remainingLockHours = Math.ceil(24 - hoursSinceNotification);
    ...
    return (
        <div className={`bg-gray-800 rounded-2xl shadow-lg flex flex-col transition-all duration-300 animate-fade-in-up hover:ring-2 hover:ring-blue-500/50 ${isNew ? 'ring-2 ring-offset-2 ring-offset-gray-900 ring-green-500' : ''}`}>
            <div className="flex justify-between items-start p-4 sm:p-6 cursor-pointer hover:bg-gray-700/50 rounded-t-2xl transition-colors" onClick={onToggleOpen}>
                    ...
                    <div className="mt-4 pt-4 border-t border-gray-700">
                        <h4 className="text-md font-semibold text-gray-300 mb-2">Actions sur la commande</h4>
                         <div className="flex flex-col sm:flex-row flex-wrap gap-2">
                            ...
                            {isNotified && (
                                <button onClick={() => onNotifyClient(order)} disabled={isReNotifyLocked} className="flex-1 bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-3 rounded-lg text-sm flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed">
                                    <BellRing size={18} /> Notifier à nouveau
                                </button>
                            )}
                        ...
                        {isReNotifyLocked && (
                            <div className="mt-3 text-center text-xs text-gray-400 flex items-center justify-center gap-2 animate-fade-in">
                                <Clock size={14} className="flex-shrink-0" />
                                <span>Prochaine notification possible dans env. {remainingLockHours}h.</span>
                            </div>
                        )}
                    </div>
                </div>
