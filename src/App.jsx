...
const getIconForHistoryAction = (action) => {
    ...
    if (action.includes('annulée')) return XCircle;
    if (action.includes('statut précédent')) return Undo2;
...
const TailwindColorSafelist = () => ( <div style={{ display: 'none' }}><span className="text-yellow-500"></span><span className="bg-yellow-500"></span><span className="text-green-500"></span><span className="bg-green-500"></span><span className="text-blue-500"></span><span className="bg-blue-500"></span><span className="text-blue-400"></span><span className="bg-blue-400"></span><span className="text-purple-600"></span><span className="bg-purple-600"></span><span className="text-gray-600"></span><span className="bg-gray-600"></span><span className="text-red-700"></span><span className="bg-red-700"></span><span className="text-yellow-400"></span><span className="text-green-400"></span><span className="text-blue-400"></span><span className="text-purple-400"></span><span className="text-gray-400"></span><span className="text-red-400"></span></div> );
    ...
    return (
        <div className="bg-gray-800 rounded-2xl shadow-lg flex flex-col transition-all duration-300 animate-fade-in-up hover:shadow-2xl hover:ring-2 hover:ring-blue-500/50">
            <div className="flex justify-between items-start p-4 sm:p-6 cursor-pointer hover:bg-gray-700/50 rounded-t-2xl transition-colors" onClick={() => setIsOpen(!isOpen)}>
                <div><h3 className="text-xl font-bold text-white">{order.clientFirstName} {order.clientLastName}</h3><div className="mt-2 space-y-1 text-sm text-gray-300">{order.clientPhone && <div className="flex items-center gap-2"><Phone size={14} className="text-gray-400"/><span>{formatPhoneNumber(order.clientPhone)}</span></div>}{order.clientEmail && <div className="flex items-center gap-2"><Mail size={14} className="text-gray-400"/><span>{order.clientEmail}</span></div>}</div></div>
                <div className="flex flex-col items-end gap-3"><span className={`px-3 py-1 rounded-full text-sm font-semibold text-white ${statusConfig.colorClass}`}>{statusConfig.label}</span><ChevronDown size={24} className={`text-gray-400 transform transition-transform duration-300 ${isOpen ? 'rotate-180' : 'rotate-0'}`} /></div>
            </div>
                ...
                <div className="p-4 sm:p-6 border-t border-gray-700">
                    ...
                    {order.history && order.history.length > 1 && (
                        <div className="mb-4">
                            <Tooltip text="Cliquez sur 'Historique' pour voir l'intégralité des événements.">
                                <h4 className="text-md font-semibold text-gray-300 mb-3 flex items-center gap-2 cursor-help">
                                    <Info size={16} className="text-gray-400" />
                                    Les 5 activités les plus récentes :
                                </h4>
                            </Tooltip>
                             <div className="space-y-2">
                                {order.history.slice().reverse().slice(0, 5).map((event, index) => {
                                    const Icon = getIconForHistoryAction(event.action);
                                    const statusCfg = Object.values(ORDER_STATUSES_CONFIG).find(s => event.action.includes(s.label));
                                    const colorClass = statusCfg?.textColor || 'text-gray-400';
                                    return (
                                        <div key={`hist-${index}`} className="flex items-center gap-3 text-sm animate-fade-in">
                                            <Icon size={16} className={colorClass} />
                                            <span className="text-gray-300 flex-grow">{event.action} par <strong>{getUserDisplayName(event.by?.email)}</strong></span>
                                            <span className="text-gray-500 text-xs ml-auto flex-shrink-0">
                                                {new Date(event.timestamp).toLocaleDateString('fr-FR', {day: '2-digit', month: '2-digit'})}
                                            </span>
                                        </div>
                                    )
                                })}
                    ...
                    <div className="mt-4 pt-4 border-t border-gray-700">
                        ...
                        <div className="flex flex-wrap gap-2 mt-4 pt-4 border-t border-gray-600">
                            <button onClick={() => onShowHistory(order)} className="bg-gray-600 hover:bg-gray-700 text-white font-bold py-2 px-3 sm:px-4 rounded-lg text-sm flex items-center justify-center gap-2"><History size={18} /> Historique</button>
                            {isAdmin && <button onClick={() => onEdit(order)} className="bg-purple-600 hover:bg-purple-700 text-white font-bold py-2 px-3 sm:px-4 rounded-lg text-sm flex items-center justify-center gap-2"><Edit size={18} /> Modifier</button>}
                            {canBeCancelled && <button onClick={() => onCancelOrder(order)} className="bg-red-800 hover:bg-red-900 text-white font-bold py-2 px-3 sm:px-4 rounded-lg text-sm flex items-center justify-center gap-2"><XCircle size={18} /> Annuler Cmd</button>}
                            {isAdmin && <button onClick={() => onDelete(order.id)} className="bg-red-600 hover:bg-red-700 text-white font-bold py-2 px-3 sm:px-4 rounded-lg text-sm flex items-center justify-center gap-2"><Trash2 size={18} /> Supprimer</button>}
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div className="bg-gray-900 text-white min-h-screen font-sans p-4 sm:p-6 lg:p-8">
            ...
            <div className="max-w-6xl mx-auto px-2 sm:px-4 lg:px-6">
                <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8">
                    <div><h1 className="text-3xl sm:text-4xl font-bold tracking-tight">AOD Tracker OS</h1><p className="text-gray-400 mt-1 text-sm sm:text-base">Suivez vos commandes d'accessoires en temps réel.</p></div>
