...
/**
 * Extrait et formate le prénom depuis une adresse email de type "prenom.nom@domaine.com".
 * @param {string} email - L'adresse email de l'utilisateur.
 * @returns {string} Le prénom avec la première lettre en majuscule, ou l'email si le format est incorrect.
 */
const getDisplayNameFromEmail = (email) => {
    if (!email || !email.includes('@')) return email || 'Utilisateur';
    try {
        const namePart = email.split('@')[0];
        const firstName = namePart.split('.')[0];
        return firstName.charAt(0).toUpperCase() + firstName.slice(1);
    } catch (error) {
        return email; // En cas d'erreur de parsing, retourne l'email complet
    }
};
                    ...
                    <div className="text-xs text-gray-400 mt-4 pt-4 border-t border-gray-700 space-y-1">
                        {order.history && order.history.length > 0 ? (
                            order.history.map((event, index) => (
                                <p key={index}>
                                    <span className="font-medium text-white">{event.action}</span> par <span className="font-medium text-white">{getDisplayNameFromEmail(event.by?.email)}</span> le {new Date(event.timestamp).toLocaleString('fr-FR', { day: 'numeric', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' })}
                                </p>
                            ))
                        ) : (
                             <p>Commandé par <span className="font-medium text-white">{getDisplayNameFromEmail(order.orderedBy?.email)}</span> le {new Date(order.orderDate).toLocaleDateString('fr-FR')}</p>
                        )}
                    </div>
...
export default function App() {
    ...
    const [selectedAdvisorFilter, setSelectedAdvisorFilter] = useState('All'); // Réintroduit pour le filtre
    ...
    // Crée dynamiquement la liste des conseillers à partir des commandes pour le filtre
    const advisorsFromOrders = useMemo(() => {
        const advisorMap = new Map();
        orders.forEach(order => {
            if (order.orderedBy && order.orderedBy.email) {
                if (!advisorMap.has(order.orderedBy.email)) {
                    advisorMap.set(order.orderedBy.email, {
                        email: order.orderedBy.email,
                        name: getDisplayNameFromEmail(order.orderedBy.email)
                    });
                }
            }
        });
    ...
    const filteredAndSortedOrders = useMemo(() => {
        ...
        // Logique de filtre par conseiller réintroduite
        if (selectedAdvisorFilter !== 'All') {
            currentOrders = currentOrders.filter(order => order.orderedBy?.email === selectedAdvisorFilter);
        }
    ...
    const getCurrentUserInfo = useCallback(() => { if (!currentUser) return null; const displayName = getDisplayNameFromEmail(currentUser.email); return { uid: currentUser.uid, email: currentUser.email, name: displayName, role: currentUser.email === ADMIN_EMAIL ? 'admin' : 'counselor' }; }, [currentUser]);
    ...
    if (showLogin) { return ( <div className="bg-gray-900 min-h-screen flex items-center justify-center"><LoginForm onLogin={handleLogin} error={loginError} onClose={() => {}} /></div> ); }
                ...
                <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8">
                    ...
                    <div className="mt-4 sm:mt-0 flex flex-wrap items-center justify-end gap-2 sm:gap-4">
                        ...
                        <div className="flex flex-wrap gap-2 sm:gap-4">
                            {isAdmin && <span className="inline-flex items-center gap-2 bg-blue-600 px-3 py-1 rounded-full text-xs sm:text-sm font-bold text-white shadow-md"><UserCheck size={16} /> Mode Admin</span>}
                ...
                {isLoading ? (
                    ...
                    <div className="grid grid-cols-1 gap-6 animate-fade-in">
                        {filteredAndSortedOrders.map((order) => (
                            <OrderCard key={order.id} order={order} onUpdateStatus={handleUpdateStatus} onEdit={handleEditOrder} onDelete={handleDeleteOrder} isAdmin={isAdmin} onShowHistory={handleShowOrderHistory} onRevertStatus={updateOrderStatus} />
                        ))}
                    </div>
                )}
            </div>
